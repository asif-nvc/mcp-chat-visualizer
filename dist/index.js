...
const API_BASE_URL = process.env.API_BASE_URL || "https://api.navigatechat.com";
...
function createServer() {
    ...
    server.registerTool("visualize_chat", {
        title: "Visualize Chat",
        description: "Visualize a conversation as a structured JSON mind map. Pass the conversation text. The tool returns exact instructions you MUST follow to produce raw JSON output. IMPORTANT: Output ONLY valid raw JSON — never output Mermaid, Markdown, or any other format.",
    ...
    registerAppTool(server, "create_public_diagram", {
        ...
        let public_id = "";
        try {
            const response = await fetch(`${API_BASE_URL}/api/chat/diagram/public`, {
                method: "POST",
    ...
    registerAppTool(server, "update_public_diagram", {
        title: "Update Public Diagram",
        description: "Update an EXISTING diagram. Use this for ANY change to a previously created diagram: adding nodes, removing nodes, renaming, restructuring, expanding branches — any modification at all. Requires the public_id from the previous create_public_diagram call. The shareable URL stays the same. You must pass the COMPLETE updated JSON (not just the diff). To modify: take the original JSON, apply the user's requested changes, then pass the full updated JSON here with the same public_id.",
        inputSchema: {
            public_id: z
                .string()
                .describe("The public_id from the original create_public_diagram response. This MUST be the same public_id to update the existing diagram."),
            json_content: z
                .union([z.string(), z.record(z.string(), z.any())])
                .describe("The complete updated diagram JSON with metadata, nodes (each with data.label, data.type, data.summary, data.hoverSummary), edges, and hierarchy"),
        },
        _meta: {
            ui: { resourceUri: DIAGRAM_RESOURCE_URI },
        },
        ...
        const parsedContent = typeof json_content === "string"
            ? JSON.parse(json_content)
            : json_content;
        const response = await fetch(`${API_BASE_URL}/api/diagram/${public_id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ json_content: parsedContent }),
        });
    ...
    server.registerTool("justify_content", {
        title: "Justify Content",
        description: "Validate and refactor JSON content to match the NavigateChat diagram structure. Pass raw or malformed diagram JSON and receive a corrected version that conforms to the expected schema.",
        inputSchema: z.object({
            user_json: z
                .string()
                .describe("The JSON string to validate and refactor into NavigateChat diagram format"),
        }),
    }, async ({ user_json }) => {
        const response = await fetch(`${API_BASE_URL}/api/chat/justify_content`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    server.registerTool("get_whiteboard", {
    ...
    registerAppTool(server, "add_diagram_to_whiteboard", {
    ...
    registerAppTool(server, "update_diagram_position", {
    ...
    server.registerTool("delete_whiteboard", {
    ...
    registerAppResource(server, DIAGRAM_RESOURCE_URI, DIAGRAM_RESOURCE_URI, { mimeType: RESOURCE_MIME_TYPE }, async () => {
        const html = await fs.readFile(path.join(DIST_DIR, "widget.html"), "utf-8");
        return {
            contents: [
                {
    ...
    registerAppResource(server, WHITEBOARD_RESOURCE_URI, WHITEBOARD_RESOURCE_URI, { mimeType: RESOURCE_MIME_TYPE }, async () => {
        const html = await fs.readFile(path.join(DIST_DIR, "whiteboard-widget.html"), "utf-8");
        return {
            contents: [
                {
